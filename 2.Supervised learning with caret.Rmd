...
wine.df = read_csv("winequality-red (2).csv")
...
```{r}
library(ggridges)
long.trainDescr = train.trainDescr = trainDescr %>%
pivot_longer (cols = fixed.acidity:alcohol,names_to = "variables", values_to = "value")

all.raw.plot = ggplot(long.trainDescr %>% filter(value < 100), aes(y =  variables, x = value)) + 
